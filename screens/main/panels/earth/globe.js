...
//-------------THREE module for moving scene(virtual trackpad)-------------
...
// CPU fallback picker (screen-space)
...
function renderStarsXYZ(starsXYZ) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    col[i*3+0] = 1; col[i*3+1] = 1; col[i*3+2] = 1; //hardcodes every stars color as white
    ra[i]  = s.ra ?? NaN;
    dec[i] = s.dec ?? NaN;
    mag[i] = s.mag ?? NaN;
    idx[i] = s.idx ?? i;
  ...
  const m = new THREE.PointsMaterial({
    ...
    size: 8.0, //control size of star
    sizeAttenuation: false//look at three documentation for other variables to add
  });
...
// Orbit set -> draw track, place sat, init arrow outward
on(MSG.ORBIT_SET, (k) => {
  try {
